 ...
 * Created on May 25, 2023, 9:03 PM
 *
 * Purpose: Using functions to initialize, sort, and search dice 
 */
...
//Function Prototypes
void intlDce(int[], const int); // Initialize Dice
void prntDce(int[], const int); // Print Dice
void swap(int&, int&); // Swap Function
void srtDce(int[], int); // Sort Dice
...
void prntDce(int a[], const int n){
    srtDce(a, n);
    for(int i = 0; i < n - 3; i++){
        cout << "Die " << i+1 << ": " << a[i] << endl;
    }
                        
}

void intlDce(int a[], const int n){
    for(int i = 0; i < n; i++){
        a[i] = rand() % 6 + 1; // [1,6] faces of dice
    }
}