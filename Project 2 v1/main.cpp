...
void line (int);
void rule (int &,int &,int &,int &,int &,int &,int &,string &,string &);
...
int  p2Np1L (char,string,int,int,int,int);
...
int main(int argc, char** argv){
    ...
    int die1,die2,die3, die4, die5, die6, // number of dice in game(3 per player)
        ...
        title=0,
        sumCount; /*sum of all dice equal to call2. I.E. If call2 = 3
                   and three dice are equal to 3, then sumCount = 9.*/
    fstream dice;
    ...
    cout << endl << "Do you want to play?" << endl;
        ...
        switch(choice) {
            case '1': { cout << endl << "Lets Play" << endl;
                do {
                    rule (die1,die2,die3,die4,die5,die6,sumCount,p1,p2);

                    // Begin Game
                    dice.open("dice.dat", ios::out);
                    dice << die1 << endl << die2 << endl
                         << die3 << endl << die4 << endl
                         << die5 << endl << die6 << endl;

                    cout << "Player 1:\n";
                    cout << "Die1: " << die1 << endl << "Die2: " << die2
                         << endl << "Die3: " << die3 << endl << endl;
                    ...
                    do {
                        ...
                        // Code for if Player 2 doesn't call Player 1 a liar
                            p2Np1L (liar,p2,call1,call2,call3,call4);
...
void rule (int &die1, int &die2,int &die3, int &die4,int &die5,int &die6,int &sumCount,string &p1,string &p2){
...
int p2Np1L (char liar,string p2,int call1,int call2,int call3,int call4){
    if (liar == 't' || liar == 'T') {
    do{
